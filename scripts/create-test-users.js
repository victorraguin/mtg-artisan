...
const supabaseUrl = process.env.VITE_SUPABASE_URL || 'https://jihtpkegbsjavmmdcbhm.supabase.co';
const supabaseServiceKey = process.env.SUPABASE_SERVICE_ROLE_KEY || 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImppaHRwa2VnYnNqYXZtbWRjYmhtIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc1NjMzMjkwOSwiZXhwIjoyMDcxOTA4OTA5fQ.QaIzq7YG4VvzKeEnI1riMwNOLtCwM0x9y235kz1eFUc';
    ...
    try {
      ...
      if (profileError) {
        if (profileError.code === '23505') { // Duplicate key
          ...
          console.error(`❌ Erreur lors de la création du profil pour ${userData.email}:`, profileError.message);
        }
      } else {
...
function getAvatarUrl(displayName) {
  const avatars = {
    'Admin User': 'https://images.pexels.com/photos/220453/pexels-photo-220453.jpeg?w=150',
    'ArtMaster Alice': 'https://images.pexels.com/photos/415829/pexels-photo-415829.jpeg?w=150',
